...
class Movies implements MatchInterface, BuildInterface
{
    public function match($routePath, \Zend_Controller_Request_Http $request, Router $router)
    {
        $action = $router->resolveActionWithIntegerParam($routePath, $request, 'movie_id');
        return array('controller' => '\App\ControllerAdmin\Movie', 'action' => $action);
    }

    public function buildLink($originalPrefix, $outputPrefix, $action, $extension, $data, array &$extraParams)
    {
        return Router::buildBasicLinkWithIntegerParam($outputPrefix, $action, $extension, $data, 'movie_id', 'title');
    }
}